...
//        determineView(viewGroup);
    ...
    protected void onPause() {
        super.onPause();
        boolean enableLock = CommonUtils.loadBooleanPreference(SettingsActivity_.this, Constants.APP_LOCK_ENABLE);
        if (enableLock) {
            long currentMillis = System.currentTimeMillis();
            CommonUtils.saveLongPreference(SettingsActivity_.this, Constants.SETTING_PAUSE_MILLIS, currentMillis);
        }

        // init current selected font
        FontUtils.setCommonTypeface(SettingsActivity_.this, getAssets());
    }
    ...
    protected void onResume() {
        super.onResume();
        boolean enableLock = CommonUtils.loadBooleanPreference(SettingsActivity_.this, Constants.APP_LOCK_ENABLE);
        long pauseMillis = CommonUtils.loadLongPreference(SettingsActivity_.this, Constants.SETTING_PAUSE_MILLIS, 0);
        if (enableLock && pauseMillis != 0) {
            if (System.currentTimeMillis() - pauseMillis > 1000) {
                // 잠금해제 화면
                Intent lockDiaryIntent = new Intent(SettingsActivity_.this, DiaryLockActivity.class);
