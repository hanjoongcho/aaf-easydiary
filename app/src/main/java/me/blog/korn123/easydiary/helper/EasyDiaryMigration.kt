...
class EasyDiaryMigration : RealmMigration {
    ...
    override fun migrate(realm: DynamicRealm, oldVersion: Long, newVersion: Long) {
        ...
        if (currentVersion == 1L) {
            val diarySchema = schema.get("DiaryDto")
        ...
        if (currentVersion == 2L) {
            val diarySchema = schema.get("DiaryDto")
            diarySchema
                    .addField("weather", Int::class.javaPrimitiveType)
        ...
        if (currentVersion == 3L) {
            ...
            val photoUriSchema = schema.create("PhotoUriDto")
        ...
        if (currentVersion == 4L) {
            val diarySchema = schema.get("DiaryDto")
            diarySchema.addField("fontName", String::class.java)
            currentVersion++
        }

        if (currentVersion == 5L) {
            val diarySchema = schema.get("DiaryDto")
            diarySchema.addField("fontSize", Float::class.javaPrimitiveType)
            currentVersion++
        }
