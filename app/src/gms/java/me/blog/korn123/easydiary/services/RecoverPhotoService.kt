...
class RecoverPhotoService : Service() {
    ...
    override fun onCreate() {
        ...
        notificationBuilder =
            NotificationCompat.Builder(applicationContext, "${NOTIFICATION_CHANNEL_ID}_download")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && notificationManager.getNotificationChannel(
                "${NOTIFICATION_CHANNEL_ID}_download",
            ) == null
        ) {
                ...
                .setStyle(
                    ...
                    if (config.enableDebugOptionVisibleAlarmSequence) {
                        "[$NOTIFICATION_FOREGROUND_PHOTO_RECOVERY_GMS_ID] ${
                            getString(
                                R.string.notification_msg_download_progress,
                            )
                        } ${successCount + failCount}/${targetItems.size}"
                    } else {
                        "${getString(R.string.notification_msg_download_progress)} ${successCount + failCount}/${targetItems.size}"
                    },
            ...
            .setContentTitle(
                if (config.enableDebugOptionVisibleAlarmSequence) {
                    "[$NOTIFICATION_GMS_RECOVERY_COMPLETE_ID] ${
                        getString(
                            R.string.recover_attach_photo_title,
                        )
                    }"
                } else {
